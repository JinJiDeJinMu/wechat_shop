...
function loginByWeixin(userInfo) {
  ...
  return new Promise(function (resolve, reject) {
    return util.login().then(res => {
      ...
      params.referrerId = wx.getStorageSync('referrerId');
      util.request(api.AuthLoginByWeixin, params, 'POST').then(res => {
        if (res.errno === 0) {
          ...
          wx.setStorageSync('merchantId', res.data.userVo.merchant_id);
          wx.setStorageSync('isDistribut', res.data.userVo.isDistribut);

          if (wx.getStorageSync('userId')) {
            wx.removeStorageSync('userId'); // wx.setStorageSync('userId');
          }
