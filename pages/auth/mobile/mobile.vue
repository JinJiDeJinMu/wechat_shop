  ...
  <view>
    ...
    <form @submit="bindLoginMobilecode">
      <form @submit="bindGetPassCode">
        <view class="login">
          <view class="first-line"><input type="digit" name="mobile" @input="bindInputMobile" placeholder="输入手机号" :value="mobile" auto-focus></input></view>
          <view class="second-line">
            <input type="digit" name="code" placeholder="六位位验证码"></input>
...
export default {
  ...
  methods: {
    ...
    bindLoginMobilecode: function (e) {
      ...
      const param = {};
      param.mobile = mobile;
      param.captcha = e.detail.value.code;
      param.inviteCode = 1;
      util.request(api.BindMobile, param, 'POST', 'application/x-www-form-urlencoded').then(function (res) {
        if (res.data.code == 200) {
          wx.showModal({
            title: '提示',
            content: '操作成功',
